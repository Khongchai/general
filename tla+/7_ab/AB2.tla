...
VARIABLES
  AVar,
  BVar,    \* The same as in module ABSpec
...
StateConstraints == ( Len(AtoB2) < 4 ) /\ ( Len(BtoA2) < 4 )
...
(***************************************************************************)
CorruptMsg ==
  /\ \/ /\ \E i \in 1 .. Len(AtoB2): AtoB2' = [AtoB2 EXCEPT ![i] = Bad]
        /\ BtoA2' = BtoA2
...
RemoveBad(seq) ==
  IF seq = <<>>
  THEN <<>>
  ELSE ( IF Head(seq) = Bad THEN <<>> ELSE << Head(seq) >> ) \o
      RemoveBad(Tail(seq))
...
(***************************************************************************)
RemoveBadAlt(seq) == LET Test(elt) == elt # Bad IN SelectSeq(seq, Test)
